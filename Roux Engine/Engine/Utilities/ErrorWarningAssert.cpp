...
//#include "Core/PlatformSpecificHeaders.hpp"
#include <stdarg.h>
#include <iostream>
...
{
	std::string fullMessageTitle = "Error";
	...
	if( conditionText )
	{
		...
		fullMessageText += _itoa( cppLineNum, buffer, 10 );
		fullMessageText += "of:\n";
...
{
	std::string fullMessageTitle = "Warning";
	std::string fullMessageText = errorMessage;
	bool isDebuggerPresent = (IsDebuggerPresent() == TRUE);
